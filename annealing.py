...
def calc_z(calcxi, calckn):
    ...
    for n in range(len(calckn)):
        z = z - (1/beta)*calckn[n]*kioskCost[n]
    return z
...
def newSolution(T):
    ...
    print("xi: ", nxi, "kn: ", nkn, "z = ", calc_z(nxi, nkn))
    
    check_Constraints(nxi, nkn)
    ...
    elif random.uniform(0,1) <= math.exp((Zn-Zc)/T):
    #elif random.uniform(0,1) <= math.exp(0.5):
        xi = copy.deepcopy(nxi)
        kn = copy.deepcopy(nkn)
        print("accepted")
    
    if calc_z(nxi, nkn) > calc_z(incXi, incKn):
        incXi = copy.deepcopy(nxi)
        incKn = copy.deepcopy(nkn)  


#Main
#set inclumbent equal to initial solution
...
print("xi: ", incXi, "kn: ", incKn, "z = ", calc_z(incXi, incKn))
